...
/*   Updated: 2023/02/23 16:17:14 by mel-aini         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	alloc_lis_indexes(t_tools *tools, int size, int index)
{
	int	i;
	int	j;
	int	k;
	...
	if (!tools->lis_indexes)
		ft_error();
	i = 0;
	while (i < size && tools->len[index][i] != tools->lis_size)
		i++;
...
void	alloc_lis(t_tools *tools, int size, int index)
{
	...
	stack = tools->best_lis;
	tools->lis = NULL;
	tools->lis = malloc(sizeof(int) * tools->lis_size);
	if (!tools->lis)
		ft_error();
	alloc_lis_indexes(tools, size, index);
	i = 0;
	j = 0;
	while (i < size)
	{
